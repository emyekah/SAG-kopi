...
class User extends CI_Controller {
	...
	public function ajax_list()
   {
      ...
      foreach ($list as $i) {
			...
			if($i->status == 1) {
				$status = '<a href="'.site_url('user/status/2/'.$i->id_user).'" class="btn btn-success btn-xs">Aktif</a>';
			} else {
				$status = '<a href="'.site_url('user/status/1/'.$i->id_user).'" class="btn btn-danger btn-xs">Non-Aktif</a>';
			}
